...
import {
  LayoutDashboard, Calendar, BookOpen, Settings, LogOut,
  ChevronLeft, ChevronRight, Zap,
} from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';

const NAV_ITEMS = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/organizer' },
  { label: 'My Events', icon: Calendar, to: '/organizer/events' },
  { label: 'Bookings', icon: BookOpen, to: '/organizer/bookings' },
  { label: 'Settings', icon: Settings, to: '/organizer/settings' },
];

export default function OrganizerSidebar({ collapsed, onToggle, mobileOpen, onMobileClose }) {
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          ...
          isActive
            ? 'bg-indigo-600/20 text-indigo-700 dark:text-indigo-300 ring-1 ring-indigo-500/30'
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      >
        <div className="flex-shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-emerald-500 flex items-center justify-center shadow-lg shadow-indigo-500/30">
          <Zap size={18} className="text-white" />
        </div>
        {!collapsed && (
          <div>
            <p className="font-bold text-slate-900 dark:text-white text-sm leading-tight">EventMate</p>
            <p className="text-[10px] text-indigo-400 font-medium tracking-widest uppercase">Organizer</p>
          </div>
        )}
      </div>
